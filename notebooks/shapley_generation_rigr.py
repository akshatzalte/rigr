...
keep_features_some = [True, False, True, True]
...
atom = mol.GetAtomWithIdx(1)  # Get the first atom
...
class CustomMultiHotBondFeaturizer(MultiHotBondFeaturizer):
    ...
    def __init__(self,
        ...
        self._MultiHotBondFeaturizer__size = 2

        if keep_features is None:
            keep_features = [True] * 1
        self.keep_features = keep_features        
...
stereos = [0, 1, 2, 3, 4, 5]
keep_features_all = [True] * 1
keep_features_some = [True]
keep_features_none = [False] * 1
...
atom_featurizer = CustomMultiHotAtomFeaturizer(
    ...
    num_Hs=[0, 1, 2],
    hybridizations=[1, 2, 3],
    keep_features=[True, True, False, True]
)
...
featurizer = CustomSimpleMoleculeMolGraphFeaturizer(
    ...
    keep_atom_features=[True, True, False, True],
    keep_bond_features=[True],
)
    ...
    def __call__(self, X):
        ...
        for keep_features in X:
            try:
                ...
                keep_atom_features = keep_features[:4] # 4 atom features
                keep_bond_features = keep_features[4:] # 1 bond features
            except:
...
for k in range(5):
    ...
    checkpoint_path = f'/home/akshatz/bond_order_free/k_means/rigr_h298_50k/run1_bof_best/chemprop_training/final_data_set/2024-08-29T11-14-39/fold_0/model_{k}/best.pt'
    ...
    keep_features = [1] * 5  # 4 atom features + 1 bond features
    feature_choice = np.array([keep_features])

    save_path = Path(f'k_means/shapley_az_rigr_model_{k}')
