...
from scikit_mol.fingerprints import MACCSKeysFingerprintTransformer
...
if __name__ == "__main__":
    ...
    # Prepare output file
    output_file = open(output_dir / "summary_maccs.md", "w")
    ...
    for seed_idx, random_seed in enumerate(random_seeds):
        ...
        pipe = Pipeline(
            [
                ...
                ("mol2fp", MACCSKeysFingerprintTransformer()),
                ("model", model),
            ]
        )
...
"""
    ...
    test_df.to_csv(output_dir / "final_test_with_maccs_preds.csv", index=False)
    aug_test_df.to_csv(output_dir / "final_aug_test_with_maccs_preds.csv", index=False)
...
"""
    ...
    output_file.close()
    print(f"Evaluation completed. Results saved to {output_dir / 'summary_maccs.md'}")
